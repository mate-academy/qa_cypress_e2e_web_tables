...
describe('Web Tables page', () => {
  ...
  it('should have a button to switch pages', () => {
    cy.contains('.-btn', 'Next').should('exist');
  });

  it('should be able to select row count', () => {
    cy.get('select').select('25 rows');
    cy.get('select').should('contain', '25 rows');
  });

  it('should be add a New Yorker.', () => {
  ...
  it('should be delete a worker', () => {
    cy.get('#delete-record-1').click();
    cy.get('.rt-table').should('not.contain', 'Cierra');
  });

  it('should be delete all workers', () => {
    cy.get('#delete-record-1').click();
  ...
  it('should be find and edit worker', () => {
  ...
  it('should be search by all column values', () => {
    cy.get('#searchBox').type('Cierra');
